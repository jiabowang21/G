...
void main() { 
	vec3 V = normalize(modelViewMatrixInverse[3].xyz-P);
  	...
  	vec3 L4 = vec3(-10, 0, 0);
  	if (rotate) {
    	mat3 R = mat3(vec3(cos(time), sin(time), 0), vec3(-sin(time), cos(time), 0), vec3(0, 0, 1));
    	L1 = R * L1;
    	L2 = R * L2;
    	L3 = R * L3;
    	L4 = R * L4;
	...
	gl_FragColor=light(V, N, P, L1, vec3(0, 1, 0)) + light(V, N, P, L2, vec3(1, 1, 0)) + light(V, N, P, L3, vec3(0, 0, 1)) + light(V, N, P, L4, vec3(1, 0, 0));
}
